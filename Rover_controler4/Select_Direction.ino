...
void Select_Direction() {
    ...
    float numer, denom;
    unsigned int sonar_dist[N+1], Hist[N+1], tgap[N+1];
    ...
    for(i =0; i <= N; i++)     // goes from 180 degrees to 0 degrees 
    {
      ...
      delay(100);                             // waits 550ms for the servo to reach the position 
    ...
    for(i=0;i < (int)border.size();i++) 
    {
      ...
      if (fabs(Delta_Angle) < 60) 
      {
        ...
        tgap[angle_cnt] = gap_type;
        rebound[angle_cnt].second = denom / (end_index-start_index+1);
        rebound[angle_cnt].first = degrees(numer/denom);
        telem << "REBOUND ANGLE = " << rebound[angle_cnt].first << " degrees";
        telem << "   Gap Type: " << tgap[angle_cnt] << endl;  
    ...
    telem << "Best Angle:  " << rebound_angle;
    telem << "   Gap Type: " << tgap[maxIndex] << endl;  
